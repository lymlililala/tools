...
const result = computed((): ConvertResult => {
  ...
  try {
    ...
    const toml = [stringifyToml(obj)].flat().join('\n').trim();
    return { toml, error: null };
  }
  catch (e: any) {
    return { toml: '', error: e?.message ?? 'Invalid JSON' };
  }
});
...
<template>
  <div class="tool-wide json-toml-wrap">
    <div class="editor-grid">
      <!-- 左：JSON 输入 -->
      <div class="pane">
        <div class="pane-header">
          <span class="pane-title">{{ t('tools.json-to-toml.jsonInput') }}</span>
          <div class="pane-actions">
            <c-button
              ...
              size="small"
              class="action-btn"
              :title="t('tools.json-to-toml.clear')"
              @click="clearInput"
            >
        ...
        <c-code-input
          v-model="jsonInput"
          language="json"
          :placeholder="t('tools.json-to-toml.jsonPlaceholder')"
      ...
      <div class="pane">
        <div class="pane-header">
          <span class="pane-title">{{ t('tools.json-to-toml.tomlOutput') }}</span>
          <div class="pane-actions">
            <c-output-actions
              :value="tomlOutput"
              :downloadable="true"
        ...
        <c-code-input
          ...
          language="toml"
          :placeholder="t('tools.json-to-toml.tomlPlaceholder')"
          min-height="calc(100vh - 218px)"
          readonly
        />
        ...
        <div v-else class="error-pane" style="min-height:420px">
          ...
          <div class="error-title">
            {{ t('tools.json-to-toml.parseError') }}
          </div>
          <pre class="error-message">{{ parseError }}</pre>
          <div class="error-tip">
            {{ t('tools.json-to-toml.errorTip') }}
          </div>
        </div>
      </div>
    </div>
