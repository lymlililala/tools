...
// 选号原则（2026-06 调整）：博客线合成最怕"时效新闻被放大成事实"触发虚构否决，
// 故**偏好常青型**：教程 / 评测 / 最佳实践 / 软件推荐 / 开源发现，**剔除纯 AI 快讯号**
// （量子位/机器之心/新智元/AI前线/夕小瑶 等已移除——其高产时效新闻是早期虚构断言的源头）。
// 同名号问题留 candidates 供人工核对后改 wxid。
const ACCOUNT_NAMES = [
  // ── 开源项目 / GitHub 工具发现（选题最贴合本站，常青、质量高）──
  'HelloGitHub', 'GitHubDaily', '开源前哨', '开源中国', 'Linux中国',
  // ── 效率工具 / 软件评测推荐（"工具"垂类核心，评测型=常青）──
  '小众软件', '少数派', 'AppSo', '异次元软件世界', '高效率工具搜罗', '利器', 'MacTalk',
  // ── 开发者综合 / 教程深度（基础/系统设计/源码，强常青）──
  '码农翻身', '稀土掘金技术社区', '小林coding', 'JavaGuide', '芋道源码',
  // ── 阮一峰（周刊型，技术/工具/趋势，质量极高、常青）──
  '阮一峰的网络日志',
  // ── 前端（本站工具多为前端可用，教程/周刊型选题契合）──
  '前端大全', '前端早读课', '张鑫旭-鑫空间-鑫生活', '奇舞周刊', '前端充电宝',
  // ── 后端 / Java / 服务端（教程、最佳实践）──
  'ImportNew', '后端技术指南针', '程序猿DD',
  // ── 移动端 ──
  '鸿洋', '郭霖',
  // ── 大厂技术团队（工程最佳实践，深度常青）──
  '美团技术团队', '阿里云开发者', '腾讯技术工程',
  // ── 数据 / Python / 机器学习（教程型，非快讯）──
  'Datawhale', 'Python猫',
]
